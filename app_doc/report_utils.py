...
class ReportEPUB():
    ...
    def write_html(self, d, html_str):
        ...
        for mindmap in mindmap_tag:
            ...
            # 脑图HTML文件路径
            temp_mindmap_html = settings.BASE_DIR + \
                '/media/report_epub/mindmap_{}.html'.format(str(time.time()))
        ...
        for tex in tex_tag:
            ...
            '''.format(content=tex)
            # 公式HTML文件路径
            temp_tex_html = settings.BASE_DIR + \
                '/media/report_epub/tex_{}.html'.format(str(time.time()))
            ...
            tex.string = ''
            tex_img_tag = html_soup.new_tag(
                name='img', src='../Images/' + tex_img_filename)
        ...
        for flowchart in flowchart_tag:
            ...
            # 流程图HTML文件路径
            temp_flow_html = settings.BASE_DIR + \
                '/media/report_epub/flow_{}.html'.format(str(time.time()))
            ...
            # 将图片标签添加进去
            flowchart.string = ''
            flow_img_tag = html_soup.new_tag(
                name='img', src='../Images/' + flow_img_filename)
        ...
        for seque in seque_tag:
            ...
            # 时序图HTML文件路径
            temp_seque_html = settings.BASE_DIR + \
                '/media/report_epub/seque_{}.html'.format(str(time.time()))
            ...
            # 将图片标签添加进去
            seque.string = ''
            seque_img_tag = html_soup.new_tag(
                name='img', src='../Images/' + seque_img_filename)
        ...
        for echart in echart_tag:
            ...
            # 脑图HTML文件路径
            temp_echart_html = settings.BASE_DIR + \
                '/media/report_epub/echart_{}.html'.format(str(time.time()))
            ...
            <html xmlns="http://www.w3.org/1999/xhtml">
              ...
              <body class="bookname">
                  <div class="main">
                    ...
                    <p class="book-src">本书籍由<a href='http://storyhub.zmister.com'>StoryHub(storyhub.zmister.com)</a>生成</p>
                  </div>
            </body>
            </html>
        '''.format(
        ...
        content_info = '''<?xml version="1.0" encoding="utf-8" ?>
            <package version="2.0" xmlns="http://www.idpf.org/2007/opf" unique-identifier="uid" >
              <metadata xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:opf="http://www.idpf.org/2007/opf">
                ...
                <dc:publisher>StoryHub制作</dc:publisher>
    ...
    def generate_epub(self):
        try:
            ...
            zipfile_name = settings.MEDIA_ROOT + \
                '/report_epub/{}'.format(self.project.name) + \
                '_'+str(int(time.time()))
            zip_name = shutil.make_archive(
                base_name=zipfile_name,
                format='zip',
                root_dir=settings.MEDIA_ROOT +
                '/report_epub/{}'.format(self.project.id)
            )
            # print(zip_name)
            # 修改zip压缩文件后缀为EPUB
    ...
    def work(self):
        ...
        for d in data:
            ...
            for d2 in data_2:
                ...
                toc_list['2'].append(
                    {'id': d2.id, 'name': d2.name, 'parent': d.id})
                # 获取第三级文档
                data_3 = Doc.objects.filter(parent_doc=d2.id).order_by("sort")
                for d3 in data_3:
                    ...
                    toc_list['3'].append(
                        {'id': d3.id, 'name': d3.name, 'parent': d2.id})
        ...
        # PDF文件路径
        report_file_path = report_pdf_folder + \
            '/{0}.pdf'.format(temp_file_name)
