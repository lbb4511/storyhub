        ...
        if user_obj:
            if user_obj.is_active:
                ...
                AppUserToken.objects.update_or_create(
                    defaults={"token": token}, user=user_obj)
                # 将token返回给用户
                res["token"] = token
    ...
    def get(self, request):
        ...
        if range == 'self':
            colla_list = [i.project.id for i in ProjectCollaborator.objects.filter(
                user=request.user)]  # 用户的协作文集列表
            project_list = Project.objects.filter(
                Q(create_user=request.user) |
                Q(id__in=colla_list)
            )
            page = PageNumberPagination()  # 实例化一个分页器
            page_projects = page.paginate_queryset(
                project_list, request, view=self)  # 进行分页查询
            serializer = ProjectSerializer(
                page_projects, many=True)  # 对分页后的结果进行序列化处理
            resp = {
                'code': 0,
            ...
            # 没有搜索 and 认证用户 and 有筛选
                ...
                if role in ['0', 0]:
                    ...
                    colla_list = [i.project.id for i in ProjectCollaborator.objects.filter(
                        user=request.user)]  # 用户的协作文集列表
                    project_list = Project.objects.filter(
                        id__in=colla_list).order_by("{}create_time".format(sort_str))
                else:
                    return Response({'code': 2, 'data': []})
        ...
        try:
            ...
            if doc_id:
                ...
                if (request.user == doc.create_user) or (colla_user_role == 1) or (request.user == project.create_user):
                    ...
                    chr_doc_ids = chr_doc.values_list(
                        'id', flat=True)  # 提取下级文档的ID
                    # 修改下级文档的状态为删除
                    chr_doc.update(
                        status=3, modify_time=datetime.datetime.now())
    ...
    def get(self, request):
        if request.auth:
            ...
            if temp_id != '':
                ...
                page_doctemps = page.paginate_queryset(
                    doctemps, request, view=self)
                serializer = DocTempSerializer(page_doctemps, many=True)
                resp = {'code': 0, 'data': serializer.data,
                        'count': doctemps.count()}
            return Response(resp)
        else:
...
class ImageView(APIView):
    ...
    def get(self, request):
        if request.auth:
            ...
            page = PageNumberPagination()
            page_images = page.paginate_queryset(
                image_list, request, view=self)
...
class ImageGroupView(APIView):
    ...
    def get(self, request):
        try:
            ...
            non_group_cnt = Image.objects.filter(
                group_id=None, user=request.user).count()
            group_list.append(
                {'group_name': '全部图片', 'group_cnt': all_cnt, 'group_id': 0})
            group_list.append(
                {'group_name': '未分组', 'group_cnt': non_group_cnt, 'group_id': -1})
    ...
    def put(self, request):
        ...
        if group_name not in ['', '默认分组', '未分组']:
            ...
            ImageGroup.objects.filter(
                id=group_id, user=request.user).update(group_name=group_name)
            return Response({'code': 0, 'data': 'ok'})
        else:
            return Response({'code': 5, 'data': '名称无效'})
    ...
    def delete(self, request):
        try:
            ...
            group = ImageGroup.objects.get(
                id=group_id, user=request.user)  # 查询分组
            images = Image.objects.filter(
                group_id=group_id).update(group_id=None)  # 移动图片到未分组
            group.delete()  # 删除分组
            return Response({'code': 0, 'data': 'ok'})
        except:
